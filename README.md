...
## A correção principal sobre a planilha

A célula `K4:K5` da aba `Set.` calcula

```
saldo = entradas − poupança − contas
```

ou seja, o saldo negativo já embute a poupança planejada. O painel separa as
...
Os dados vivem em três arrays no `<script>` do final de `index.html`:
`contas`, `entradas` e `poupanca`. Substitua os valores pelos da aba do mês
desejado e ajuste as constantes `ENTRADAS_TOTAL` e `CONTAS_TOTAL`, junto dos
números citados no texto das seções.
